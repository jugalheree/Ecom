...
export default function Register() {
  ...
  const [form, setForm] = useState({
    name: "", email: "", phone: "", password: "", role: "BUYER", isB2B: false,
  });
  ...
  async function handleRegister(e) {
    ...
    try {
      ...
      login({ user: data.user, role: data.user.role.toLowerCase() });

      // Sync any guest cart items
      ...
      showToast({ message: error.response?.data?.message || error.message || "Registration failed", type: "error" });
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="hidden lg:flex flex-col justify-between w-[42%] bg-gradient-to-br from-navy-950 via-ink-950 to-ink-900 p-12 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-6">
          <h2 className="text-4xl font-display font-bold text-white leading-tight">
            Your marketplace<br /><span className="text-brand-400 italic font-light">journey starts here.</span>
          </h2>
          <p className="text-ink-400 text-sm leading-relaxed max-w-xs">Whether you're shopping or selling, TradeSphere gives you the tools, trust, and technology to succeed.</p>
          <div className="space-y-3 pt-2">
            {[{ icon: "🔒", text: "Escrow-protected payments" }, { icon: "🤖", text: "AI Trust Score for every product" }, { icon: "📊", text: "Real-time analytics dashboard" }, { icon: "💬", text: "Dedicated vendor support" }].map((f, i) => (
      ...
      <div className="flex-1 flex items-center justify-center p-6 bg-sand-50 overflow-y-auto">
        <div className="w-full max-w-md py-8">
          ...
          <div className="grid grid-cols-3 gap-2.5 mb-6">
            {roles.map((r) => (
              <button key={r.value} type="button"
                onClick={() => setForm((f) => ({ ...f, role: r.value, isB2B: false }))}
                className={`p-4 rounded-xl border-2 text-left transition-all duration-150 ${form.role === r.value ? "border-brand-500 bg-brand-50 shadow-soft" : "border-ink-200 bg-white hover:border-ink-300"}`}>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            {/* Vendor type info note */}
            {form.role === "VENDOR" && (
              <div className="p-4 bg-brand-50 rounded-xl border border-brand-200">
                <p className="text-sm font-semibold text-brand-800 mb-1">🏪 Vendor Account</p>
                <p className="text-xs text-brand-700 leading-relaxed">After registration you'll choose between <strong>Solo Vendor</strong> (individual seller) or <strong>Organization Vendor</strong> (registered business) and set up your shop profile.</p>
              </div>
            )}
