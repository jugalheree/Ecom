    ...
    <div className="min-h-screen bg-white">
      <div className="container-app py-12">
        ...
        <div className="space-y-6">
          {deliveries.length === 0 ? (
            <Card className="p-8 text-center text-stone-500 border-2 border-stone-200">No deliveries to track.</Card>
          ) : (
            deliveries.map((d) => (
              <Card key={d._id} className="p-6 border-2 border-stone-200 hover:border-primary-300 transition-all duration-300">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <p className="text-stone-600">{d.address}</p>
                  </div>
                    ...
                    return (
                      <div
                        ...
                        className={`flex-shrink-0 px-3 py-2 rounded-lg text-xs font-medium ${
                          active ? "bg-primary-100 text-primary-800" : "bg-stone-100 text-stone-500"
                        }`}
                      >
                        {s.replace(/_/g, " ")}
                      </div>
                ...
                {d.status === "DELIVERED" && (
                  <div className="mt-4 pt-4 border-t border-stone-200">
                    {d.proofUrl ? (
                      <p className="text-sm text-stone-600">Proof: {d.proofUrl}</p>
                    ) : (
                      <Button
                        variant="outline"
