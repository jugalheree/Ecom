...
export default function VendorOrders() {
  ...
  const [rejectModal, setRejectModal] = useState(null); // { returnId }
                  ...
                  {isExpanded && (
                    <div className="border-t border-ink-100 divide-y divide-ink-50">
                      {order.items?.map((item, i) => {
                        // Match product details from the aggregation lookup
                        ...
                        // returnId would need to come from order.items if embedded — fall back to item._id
