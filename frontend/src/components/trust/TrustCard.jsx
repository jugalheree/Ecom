export default function TrustCard({ title, rating, reviews, badge }) {
  return (
    <div className="bg-white border border-ink-200 rounded-2xl p-5 hover:shadow-card-hover hover:border-ink-300 transition-all">
      <p className="text-sm text-ink-500 font-medium">{title}</p>

      <div className="flex items-center gap-2 mt-2">
        <p className="text-2xl font-bold text-ink-900">{rating.toFixed(1)}</p>
      ...
      <p className="text-sm text-ink-600 mt-1">{reviews} reviews</p>
